...
Web UI の起動中に認証情報が期限切れになった場合は、次のS3アクセス時に認証エラーを検知して S3 クライアントを作り直し、1回だけ自動で再試行します。AWS SSO のキャッシュや `~/.aws/credentials`、`.env` の一時認証情報を裏で更新した場合、Web UIを再起動せずに復帰できることがあります。
