...
public リポジトリとして公開する前、または社外共有前には以下を確認してください。

- `.env`, `.env.local`, `.s3-work/`, `dist/`, `node_modules/` をコミットしない
- README やスクリーンショットに実バケット名、アカウントID、社内パスが残っていないか確認する
- `--allow-write` と `--allow-create-bucket` は必要な時だけ有効にする
- AWS S3 接続時は、対象バケットと Prefix を確認してからアップロードする
- 必要最小限の IAM 権限で実行する
